...
Creates a Kubernetes PVC in the release namespace for every synced group entry (objectClass `groupOfNames` by default). Idempotent via label-selector lookup. RBAC: get/list/create on PVCs, gated by the `ldap-sync.pvcGroupEnabled` helper.
...
Admin connection params (host, port, username, password, database, sslmode) come directly from the plugin's Helm values; the `postgres` admin account is assumed. Identifiers are quoted with `pq.QuoteIdentifier` and the password literal with `pq.QuoteLiteral` since DDL can't use bind parameters. RBAC: get/list/create/update on Secrets, gated by the `ldap-sync.postgresUserEnabled` helper.
