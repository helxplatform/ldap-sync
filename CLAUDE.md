...
Plugins run out-of-band side effects after a successful target-LDAP write. They are compiled into the main binary (not separate services like hooks). The system lives in `plugin.go` (the `Plugin` interface and dispatching `Registry`), with each plugin in its own `plugin_*.go` file and registered via the `buildPlugin` factory in `plugin_log.go`.

A `Plugin` implements `Name()`, `Match(SyncEvent) bool`, and `Apply(ctx, SyncEvent) error`. After each successful write, `markSyncedAndRelease` dispatches a `SyncEvent` (DN, Content, Op, etc.) to every matching plugin in its own goroutine with bounded retry. Plugin failures are logged but never block sync. `Apply` must be idempotent because it can be retried and re-fired on subsequent syncs.

Configuration flows from Helm values (`config.plugins.*`) → ConfigMap → `PluginsConfig` → `buildPlugin`, which decodes the per-plugin `options` map into the plugin's config struct via `decodeOptions`.

### pvc-group plugin (`plugin_pvc.go`)

Creates a Kubernetes PVC in the release namespace for every synced group entry (objectClass `groupOfNames` by default). Idempotent via label-selector lookup. RBAC: list/create on PVCs (no `get` — it's never called by name), gated by the `ldap-sync.pvcGroupEnabled` helper.

### postgres-user plugin (`plugin_postgres.go`)

Provisions Postgres credentials for every synced **user** entry (objectClass `helxUser` by default — the specific class on synced users, configurable via `matchObjectClass`). On each matching event it:

- Derives the username from the `uid` attribute (falls back to the leading `uid=` RDN), sanitized to a safe `[a-z0-9_]` identifier.
- Reads the user's group memberships from the merged `groups` attribute and maps them to Postgres roles via the `groupRoles` config. The `users` group is **required** and acts as the default role everyone receives.
- Ensures each configured role exists (NOLOGIN group role) and applies its `privileges` — plain SQL statements run verbatim (operator-trusted config, no templating).
- Ensures the user's LOGIN role with an autogenerated password, then **reconciles** memberships: GRANTs roles for current groups and REVOKEs plugin-managed roles the user no longer qualifies for. Only roles named in `groupRoles` are ever touched.
- Provisions a Kubernetes Secret (named `<namePrefix><username>`) with fixed keys `dbname`, `host`, `password`, `port`, `uri`, `user` for mounting into the user's pods. **Create-once / reuse-password**: an existing Secret's password is reused (ALTER ROLE keeps Postgres in sync) so mounted pods are never broken; a fresh password is generated only on first creation.

Admin connection params (host, port, username, password, database, sslmode) come directly from the plugin's Helm values; the `postgres` admin account is assumed. Identifiers are quoted with `pq.QuoteIdentifier` and the password literal with `pq.QuoteLiteral` since DDL can't use bind parameters. RBAC: get/create/update on Secrets (no `list` — it's never called), gated by the `ldap-sync.postgresUserEnabled` helper.
